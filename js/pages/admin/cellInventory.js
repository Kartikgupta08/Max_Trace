 ...
 *
 * Fix: Frontend field names now match actual API response shape:
 *   row.brand          (was row.model   → "N/A" every row)
 *   row.capacity       (new — mAh)
 *   row.lot            (new)
 *   row.specification  (new)
 *   row.ocv_voltage_mv (new)
 *   row.final_result   (new — grading result badge)
 *   row.ng_count       (new)
 *   row.last_test_date (new)
 *
 * API response shape (200):
 * {
 *   cell_id, brand, status, grading_status, ng_count,
 *   registered_at, last_test_date, sorting_date,
 *   ir, voltage, capacity, lot, specification,
 *   ocv_voltage_mv, final_result
 * }
 */
...
function _badge(val) {
    ...
    let style = GREY, dot = '';
    switch (v) {
        case 'GRADED':    case 'PASS': style = GREEN;  dot = '● '; break;
        case 'ASSIGNED':               style = BLUE;   dot = '● '; break;
        case 'REGISTERED':             style = ORANGE; dot = '○ '; break;
        case 'FAILED':    case 'FAIL': style = RED;    dot = '● '; break;
    }
...
const CellInventory = {
    ...
    render() {
        ...
        <style>
            ...
            .ci-header h1 { font-size: var(--text-xl); font-weight: var(--weight-bold); color: var(--color-text-primary); margin: 0 0 4px; line-height: 1.25; }
            .ci-header p  { font-size: var(--text-sm); color: var(--color-text-secondary); margin: 0; }
            ...
            @media (max-width: 1100px) { .ci-filter-grid { grid-template-columns: 1fr 1fr 1fr; } .ci-btn-col { grid-column: 1 / -1; } }
            @media (max-width: 640px)  { .ci-filter-grid { grid-template-columns: 1fr; } .ci-btn-col { grid-column: 1; } .ci-filter-card { padding: 16px 16px 12px; } }

            .ci-field { display: flex; flex-direction: column; gap: 5px; min-width: 0; }
            .ci-label { font-size: 11px; font-weight: 700; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 0.7px; white-space: nowrap; }
            .ci-input-wrap { position: relative; display: flex; align-items: center; }
            .ci-input-icon { position: absolute; left: 10px; color: var(--color-text-tertiary); display: flex; align-items: center; pointer-events: none; z-index: 1; }
            ...
            .ci-ctrl:focus { border-color: var(--color-primary); box-shadow: 0 0 0 3px var(--color-primary-surface); }
            .ci-ctrl::placeholder { color: var(--color-text-tertiary); }
            .ci-ctrl--icon { padding-left: 34px; }
            .ci-ctrl.ci-select { appearance: none; -webkit-appearance: none; cursor: pointer; background-image: url("data:image/svg+xml,%3Csvg width='11' height='7' viewBox='0 0 11 7' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4.5 4.5L10 1' stroke='%238B90A0' stroke-width='1.6' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E"); background-repeat: no-repeat; background-position: right 11px center; padding-right: 30px; }

            .ci-search-btn {
                display: inline-flex; align-items: center; justify-content: center; gap: 7px;
                height: 40px; width: 100%; padding: 0 20px;
            ...
            .ci-clear-row { display: flex; justify-content: flex-end; margin-top: 10px; }
            .ci-clear-btn { background: none; border: none; font-family: var(--font-family); font-size: 12px; color: var(--color-text-tertiary); cursor: pointer; display: inline-flex; align-items: center; gap: 4px; padding: 0; transition: color 140ms ease; }
            .ci-clear-btn:hover { color: var(--color-text-primary); }

            .ci-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 12px; margin-bottom: 16px; }
            .ci-stat { background: var(--color-bg-card); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 14px 18px; box-shadow: var(--shadow-sm); }
            .ci-stat-label { font-size: 11px; font-weight: 700; color: var(--color-text-tertiary); text-transform: uppercase; letter-spacing: 0.7px; margin-bottom: 4px; }
            .ci-stat-value { font-size: 22px; font-weight: 700; color: var(--color-text-primary); line-height: 1.2; }
            .ci-stat-sub   { font-size: 11px; color: var(--color-text-tertiary); margin-top: 2px; }

            .ci-card { background: var(--color-bg-card); border: 1px solid var(--color-border); border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); overflow: hidden; }

            .ci-card-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; border-bottom: 1px solid var(--color-border-light); flex-wrap: wrap; gap: 8px; }
            .ci-card-title  { font-size: 11px; font-weight: 700; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 1px; margin: 0; }
            .ci-count       { font-size: 11px; font-weight: 600; color: var(--color-text-tertiary); background: var(--color-bg-body); border: 1px solid var(--color-border); border-radius: 20px; padding: 2px 10px; }

            .ci-scroll { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }

            .ci-table { width: 100%; border-collapse: collapse; min-width: 900px; }
            .ci-table thead { background: var(--color-bg-table-header); position: sticky; top: 0; z-index: 2; }
            .ci-table th { padding: 10px 14px; text-align: left; font-size: 11px; font-weight: 700; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 0.8px; border-bottom: 1px solid var(--color-border); white-space: nowrap; }
            .ci-table td { padding: 11px 14px; font-size: var(--text-sm); color: var(--color-text-primary); border-bottom: 1px solid var(--color-border-light); vertical-align: middle; white-space: nowrap; }
            .ci-table tbody tr:last-child td { border-bottom: none; }
            .ci-table tbody tr { transition: background 120ms ease; }
            .ci-table tbody tr:hover td { background: var(--color-bg-table-row-hover); }

            .ci-mono { font-family: var(--font-mono); font-size: 12px; font-weight: 500; }
            .ci-num  { font-family: var(--font-mono); font-size: 12px; color: var(--color-text-secondary); text-align: right; }
            .ci-table th.ci-th-num { text-align: right; }

            @keyframes ci-shimmer { 0% { background-position: -800px 0; } 100% { background-position: 800px 0; } }
            .ci-skel { display: inline-block; height: 12px; border-radius: 4px; background: linear-gradient(90deg, var(--color-border-light) 25%, var(--color-border) 50%, var(--color-border-light) 75%); background-size: 1600px 100%; animation: ci-shimmer 1.4s ease-in-out infinite; }

            .ci-empty { text-align: center; padding: 52px 24px; color: var(--color-text-tertiary); }
            .ci-empty svg { opacity: 0.22; margin-bottom: 14px; }
            .ci-empty-ttl { font-size: var(--text-base); font-weight: 600; color: var(--color-text-secondary); margin: 0 0 4px; }
                ...
                <div class="ci-filter-grid">
                    <div class="ci-field">
                        ...
                        <div class="ci-input-wrap">
                            ...
                            <input type="text" id="ci-cell-id" class="ci-ctrl ci-ctrl--icon" placeholder="Scan or type Cell ID…" autocomplete="off" spellcheck="false"/>
                        </div>
                    </div>
                    <div class="ci-field">
    ...
    async init() {
        ...
        document.getElementById('ci-clear').addEventListener('click', () => {
            ['ci-cell-id','ci-dfrom','ci-dto','ci-brand','ci-status'].forEach(id => {
        ...
        async function _fetch(p) {
            ...
            if (cellId) {
                ...
                if (brand)  params.brand     = brand;   // ← FIX: was "model", API param is "brand"
                if (status) params.status    = status;
            }
        ...
        function _renderTable(items) {
            ...
            const rows = items.map(row => `
                <tr>
                    ...
                    <td>${row.brand || '<span style="color:var(--color-text-tertiary)">—</span>'}</td>
                    <td>${row.lot   || '<span style="color:var(--color-text-tertiary)">—</span>'}</td>
                    <td>${row.specification || '<span style="color:var(--color-text-tertiary)">—</span>'}</td>
                    <td>${_badge(row.status)}</td>
                    <td>${_badge(row.final_result)}</td>
                    <td class="ci-num">${row.capacity  != null ? Number(row.capacity).toLocaleString()    + ' mAh' : '—'}</td>
                    ...
                    <td class="ci-num">${row.ocv_voltage_mv != null ? Number(row.ocv_voltage_mv).toFixed(0) + ' mV' : '—'}</td>
                    <td style="color:var(--color-text-secondary);font-size:12px">${_fmtDate(row.registered_at)}</td>
                    <td style="color:var(--color-text-secondary);font-size:12px">${_fmtDate(row.last_test_date)}</td>
                </tr>`).join('');
        ...
        function _skeleton() {
            const el = document.getElementById('ci-results');
            const sk = ws => `<tr>${ws.map(w => `<td><span class="ci-skel" style="width:${w}"></span></td>`).join('')}</tr>`;
            el.innerHTML = `
                <table class="ci-table">
                    <thead><tr>
                        <th>Cell ID</th><th>Brand</th><th>Lot</th><th>Specification</th>
                        <th>Status</th><th>Grade Result</th>
                        <th class="ci-th-num">Capacity</th><th class="ci-th-num">Voltage</th>
                        <th class="ci-th-num">IR (mΩ)</th><th class="ci-th-num">OCV (mV)</th>
                        <th>Registered At</th><th>Last Tested</th>
                    </tr></thead>
                    <tbody>
                        ${sk(['80px','80px','70px','100px','70px','60px','60px','60px','60px','60px','100px','100px'])}
                        ${sk(['90px','70px','80px','110px','80px','60px','60px','60px','60px','60px','100px','100px'])}
                        ${sk(['75px','90px','65px','90px', '70px','60px','60px','60px','60px','60px','100px','100px'])}
                        ${sk(['85px','75px','75px','105px','65px','60px','60px','60px','60px','60px','100px','100px'])}
                    </tbody>
                </table>`;
        }
...
function _emptyHtml(msg) {
    return `<div class="ci-empty">
        <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4.03 3 9 3s9-1.34 9-3V5"/><path d="M3 12c0 1.66 4.03 3 9 3s9-1.34 9-3"/></svg>
